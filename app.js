...
/* ── Copy buttons ── */
function initCopy() {
  // Existing code-block copy buttons (Docker tab)
  ...
  // Terminal copy buttons — strip prompts/comments so the user pastes only
  // executable lines. Shows a brief visual confirmation in the same button.
  document.querySelectorAll('.terminal-copy').forEach(btn => {
    btn.addEventListener('click', async () => {
      const target = document.getElementById(btn.dataset.target);
      if (!target) return;
      const lines = target.innerText.split('\n')
        .map(l => l.replace(/^\s*\$\s?/, ''))   // remove leading "$ " prompt
        .filter(l => l.trim() && !l.trim().startsWith('#'));   // drop comments + blanks
      const cmd = lines.join('\n');
      try {
        await navigator.clipboard.writeText(cmd);
        const original = btn.innerHTML;
        btn.classList.add('copied');
        btn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><polyline points="20 6 9 17 4 12"/></svg>`;
        setTimeout(() => { btn.classList.remove('copied'); btn.innerHTML = original; }, 1700);
